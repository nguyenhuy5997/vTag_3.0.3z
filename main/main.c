...
CENG:
	...
	{
		if(cell_number_scanned(CENG_buf) < 5 && retry++ < 20)
		{
...
void Check_accuracy(void)
{
	...
	if(VTAG_Configure.Accuracy > 200)
	{
		GPS_Scan_Number = 60;
...
void BackUp_UnsentMessage(VTAG_MessageType Mess_Type)
{
	...
	if(Flag_location_send == true && ap_count > 3)
	{
		AT_RX_event = EVEN_OK;
		GPS_Scan_Number = 60;
		GPS_Operation_Thread();
...
void MQTT_Location_Payload_Convert(void)
{
	...
	} else if(GPS_Fix_Status == 1)
	{
		...
		if(GPS_Select == GPS_NMEA)
		{
			if(GPS_Fix_Status == 1)
			{
				GPS_Position_Time_Str_Convert(Mqtt_TX_Str, VTAG_DeviceParameter.Bat_Level, hgps.dop_p, hgps.latitude, hgps.longitude, hgps.altitude, 2000 + hgps.year, hgps.month, hgps.date, hgps.hours, hgps.minutes, hgps.seconds);
				ESP_LOGI(TAG, "%s\r\n", Mqtt_TX_Str);
			}
		}
...
void Bat_Wifi_Cell_Convert(char* str, char* wifi, uint8_t level, int MCC,int16_t RSRP, int16_t RSRQ, int RSSI,int cellID,int LAC,int MNC)
{
	if(Flag_LBS_need)
	{
		if(Network_Type == GSM)
		{
			sprintf(str,"{\"ss\":%d,\"Type\":\"DWFC\",\"B\":%d,\"r\":%d,\"C\":%s,\"T\":%lld,\"V\":\"%s\",\"RR\":%d%d,", RSSI, level, RSRQ, cell_buffer, VTAG_DeviceParameter.Device_Timestamp, VTAG_Vesion, Reboot_reason, Backup_reason);
			printf("cell_buffer: %s\r\n", str);
		}
		else
		{
			sprintf(str,"{\"ss\":%d,\"Type\":\"DWFC\",\"B\":%d,\"r\":%d,\"C\":%s,\"T\":%lld,\"V\":\"%s\",\"RR\":%d%d,", RSRP, level, RSRQ, cell_buffer, VTAG_DeviceParameter.Device_Timestamp, VTAG_Vesion, Reboot_reason, Backup_reason);
		}
		if (Network_Type == GSM)
		{
 ...
 void main_task(void *arg)
{
    ...
    while (1)
    {
		...
		for(int i = 0; i < Backup_Array_Counter; i++)
		{
			...
			{
				if((Flag_motion_detected == false && Backup_Array_Counter > 0 &&  esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_TIMER) && (strstr(Location_Backup_Array[i], "DBF") || strstr(Location_Backup_Array[i], "DASP")))
				{
